...
lab = {'FA','MSD', 'MD', 'RD', 'AD', 'MSK','NDI','ODI','F_{iso}'};
...
%% Global WM
gd = [FA_wm MDdf_wm  MD_wm RD_wm AD_wm MKdf_wm ND_wm OD_wm Fiso_wm];
...
fprintf(fp,'Participant,gFA,gMD, gMD, gRD, gAD,gMSKI,gODI,gNDI,gFiso\n');
for s=1:size(gd,1)
    fprintf(fp,'%d',s);
    for m=1:Nmes
...
fprintf(fp,'Participant, ROI, FA, MSD, MD, RD, AD, MSK, NDI, ODI, Fiso, \n');
for s=1:size(gd,1)
    for r=1:Nroi
