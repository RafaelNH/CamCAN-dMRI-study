...
%% Global WM
gd = [FA_wm MDdf_wm MKdf_wm ND_wm OD_wm Fiso_wm];
...
fprintf(fp,'Participant,gFA,gMD,gMSKI,gODI,gNDI,gFiso\n');
for s=1:size(gd,1)
    fprintf(fp,'%d',s);
    for m=1:6
...
lab2 = {'A)  FA','B)  MSD','C)  MSK','D)  NDI','E)  ODI','F)  F_{iso}'};
figure(2); clf
for m = 1:Nmes
    subplot(2,3,m)
...
fprintf(fp,'Participant, ROI, FA, MSD, MSK, NDI, ODI, Fiso\n');
for s=1:size(gd,1)
    for r=1:Nroi
...
axis off
for x = 1:6
    text(x-0.2, 6.8, lab{x},'Color','k')
    text(-0.2, x, lab{x}, 'Color', 'k')
end
line([0 6.5],[0 6.5],'Color','k','LineWidth',2)
...
for f = 1:3
    ...
    figure(f2), subplot(4,1,f),hold on
    ...
    axis([0.5 27.5 -0.1 0.1]);
    set(gca,'XTick',[],'YTick',[-0.1 0 0.1]);
    
    figure(f3), subplot(4,1,f),hold on
